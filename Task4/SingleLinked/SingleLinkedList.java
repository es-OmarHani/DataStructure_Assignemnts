...
//Interface that will make Our class Inherit from it all abstract methods 
...
public class SingleLinkedList implements ILinkedList {
    ...
    public void remove(int index) {
        ...
        //Create trav pointer to point on previous node of needed node
        int i;
        Node trav = head ;

        //If index = 0 that means we need to add remove from First of list
        if(index == 0 ){    
            //Trav will be pointing to nothing because first node previous of it is null
            trav = null ;
            //calling method that will remove first node
            removeFirst();
            
            return ;
        }

        //If index will be = 1 means we need to make trav point to head 
        else if(index == 1)
            trav = head;
        ...
        //calling method that will remove node by pass to it node before needed removed node
        removeNode(trav);
